...
//	GPIO_PORTF_AHB_AHB_AFSEL_R |= SOUT_TTL;										// enable alt fn
//	GPIO_PORTF_AHB_AHB_PCTL_R &= ~(GPIO_PCTL_PF1_M);
//	GPIO_PORTF_AHB_AHB_PCTL_R |= (GPIO_PCTL_PF1_SSI1TX);
...
//	SSI1_CR1_R = SSI_CR1_SSE;											// enable SSI
	// SIN config
//	j = GPIO_PORTF_AHB_AHB_IM_R;												// disable SIN edge intr
//	GPIO_PORTF_AHB_AHB_IM_R = 0x00;
//	GPIO_PORTF_AHB_IEV_R &= ~SIN_TTL;										// falling edge
//	GPIO_PORTF_AHB_IBE_R &= ~SIN_TTL;										// one edge
//	GPIO_PORTF_AHB_IS_R &= ~SIN_TTL;										// edge ints
//	GPIO_PORTF_AHB_ICR_R = 0xff;											// clear int flags
//	j |= (SIN_TTL);														// enable SIN edge intr
//	GPIO_PORTF_AHB_IM_R = j;
//	GPIO_PORTF_AHB_IM_R |= (SIN_TTL);										// enable SIN edge intr
...
void flush_sin(void){
	...
	GPIO_PORTQ_ICR_R = (DATA2);						// clear int flags
...
void gpioq_isr(void){
	if(GPIO_PORTQ_MIS_R & (DATA2)){
		GPIO_PORTQ_ICR_R = (DATA2);							// clear int flags
		GPIO_PORTQ_IM_R &= ~(DATA2);						// disable SIN edge intr
//		sin_dr = 0x00000000L;								// init data reg and mask
//		sin_mask = SIN_START;
		TIMER2_TAILR_R = (uint16_t)(SIN_HALF_BIT_TIME);
		TIMER2_CTL_R |= (TIMER_CTL_TAEN);					// enable timer
		NVIC_EN0_R = NVIC_EN0_TIMER2A;
	}else{
		GPIO_PORTQ_ICR_R = 0xff;
	}
	return;
}
...
//	This ISR times 1/2 bit time for start bit at 4800 baud, then sends 5 nybbles
//	to SSI3 to trigger its RX logic.  SSI3 EOTISR captures the RX data.
//
//-----------------------------------------------------------------------------
...
void Timer2A_ISR(void)
{
	...
	for(i=0; i<5; i++){
		SSI3_DR_R = 0;										// send 5 nybbles (20 bits)
	}
	TIMER2_CTL_R |= (TIMER_CTL_TAEN);						// disable timer
	return;
}
...
void SSI3_ISR(void)
{
	...
	for(i=0; i<5; i++){
		dr <<= 4;
		dr |= (SSI3_DR_R & 0x0f);							// gather 5 nybbles (20 bits)
	}
	sin_buf[sin_hptr++] = dr;								// put rcvd word into buffer
	if(sin_hptr >= SIN_MAX){								// process head wrap-around
		sin_hptr = 0;
	}
	GPIO_PORTQ_IM_R |= (DATA2);								// ensable SIN edge intr (gpioq)
	SSI3_ICR_R = SSI_ICR_EOTIC;							// clear isr flag
	return;
}
